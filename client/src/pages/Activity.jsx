        ...
        try {
            ...
            if (activeTab === 'report') {
                ...
                const tableRows = expectedData.map(item => {
                    ...
                    const breakdownStr = `${b.basic || 500} + ${b.devFee || 20}${b.loanPrincipal > 0 ? ' + ' + b.loanPrincipal.toFixed(0) : ''}${b.loanInterest > 0 ? ' + ' + b.loanInterest.toFixed(0) : ''}`;
                                ...
                                <table className="data-table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th className="text-left">Loan Repayment</th>
                                    ...
                                    <tbody>
                                        {reportRows.map(item => (
                                            <tr key={item.memberId}>
                                                ...
                                                <td className="text-left">{formatCurrency(item.depositAmount)}</td>
                                                ...
                                                <td style={{ fontWeight: 600, color: '#059669' }} className="text-left">
                                                    {formatCurrency(item.totalPaid)}
                                                </td>
                                            </tr>
                                        ))}
                            ...
                            <table className="data-table">
                                <thead>
                                    <tr>
                                        <th>Acc No.</th>
                                        <th>Member Name</th>
                                        <th className="text-left">Base Fee</th>
                                        <th className="text-left">Loan Due</th>
                                        <th className="text-left">Total Expected</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {expectedData.map(item => (
                                        <tr key={item.memberId}>
                                            ...
                                            <td className="text-left">{formatCurrency(item.baseAmount)}</td>
                                            <td className="text-left">
                                                {item.hasActiveLoan ? (
                                                    <span style={{ color: '#ea580c', fontWeight: 500 }}>{formatCurrency(item.loanAmount)}</span>
                                                ) : (
